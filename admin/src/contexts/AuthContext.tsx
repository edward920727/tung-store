...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (firebaseUser) => {
      console.log('[AuthContext] onAuthStateChanged 觸發，Firebase 用戶:', firebaseUser?.email || '未登入');
      setFirebaseUser(firebaseUser);
      
      if (firebaseUser) {
        // 從 Firestore 獲取用戶信息
        try {
          console.log('[AuthContext] 嘗試獲取用戶數據，UID:', firebaseUser.uid, 'Email:', firebaseUser.email);
          let userData = await firestoreService.getUser(firebaseUser.uid);
          
          // 如果通過 UID 找不到，嘗試通過 email 查找
          if (!userData && firebaseUser.email) {
            console.log('[AuthContext] 通過 UID 找不到，嘗試通過 email 查找:', firebaseUser.email);
          ...
          console.log('[AuthContext] 獲取到的用戶數據:', userData);
          if (userData) {
            console.log('[AuthContext] 用戶角色:', userData.role, '是否為管理員:', userData.role === 'admin');
            // 獲取會員等級信息並添加到用戶對象
            let membership = null;
            if (userData.membership_level_id) {
              ...
              console.log('[AuthContext] 用戶沒有會員等級 ID，設置默認等級');
              const defaultLevels = await firestoreService.getMembershipLevels();
              if (defaultLevels.length > 0) {
                await firestoreService.updateUser(userData.id, { 
                  membership_level_id: defaultLevels[0].id 
            ...
            console.log('[AuthContext] 設置用戶數據（含會員信息）:', {
              email: userWithMembership.email,
              role: userWithMembership.role,
              isAdmin: userWithMembership.role === 'admin'
            });
